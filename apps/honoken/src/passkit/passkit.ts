...
if (process.env.VERCEL !== undefined) {
  import('passkit-generator').then(
    ...
    (error) => {
      ...
      console.warn('Failed to preload passkit-generator during cold start:', error.message);
    }
  );
}
...
import { loadCertBundle, type CertBundle } from './certs';
import type { Logger } from '../utils/logger';
import { z, type ZodIssue } from 'zod/v4';
import { PassDataEventTicketSchema } from '../schemas';
import { UpstashAssetStorage } from '../storage/upstash-storage';

async function fetchVerifiedPngAsset(
  storage: UpstashAssetStorage,
  key: string,
  expectedW: number,
  expectedH: number,
  logger: Logger
): Promise<ArrayBuffer> {
  logger.info(`[fetchVerifiedPngAsset] Attempting to fetch and verify PNG asset: ${key}`, { expectedW, expectedH });

  // Fetch full asset from Redis (Upstash doesn't support partial reads)
  ...
  logger.info(`[fetchVerifiedPngAsset] Retrieved asset ${key}, size: ${fullArrayBuffer.byteLength} bytes`);

  // Fast validation: check only first 24 bytes for PNG header before processing full buffer
  if (fullArrayBuffer.byteLength < 24) {
    const errMsg = `[fetchVerifiedPngAsset] File too short for PNG validation: ${key}`;
    logger.error(errMsg, new Error(errMsg), { key, length: fullArrayBuffer.byteLength });
    throw new Error(`File too short for PNG validation: ${key}`);
  }

  // Create view of just the first 24 bytes for header validation
  const headerBytes = new Uint8Array(fullArrayBuffer, 0, 24);
  ...
  const chunkLength = (headerBytes[8] << 24) | (headerBytes[9] << 16) | (headerBytes[10] << 8) | headerBytes[11];
  ...
  if (headerBytes[12] !== 0x49 || headerBytes[13] !== 0x48 || headerBytes[14] !== 0x44 || headerBytes[15] !== 0x52) {
  ...
  const headerView = new DataView(headerBytes.buffer, headerBytes.byteOffset + 16, 8);
  ...
  if (w !== expectedW || h !== expectedH) {
    ...
    logger.error(errMsg, new Error(errMsg), { key, actualW: w, actualH: h, expectedW, expectedH });
    throw new Error(`Wrong dimensions ${w}×${h} for ${key}, expected ${expectedW}×${expectedH}`);
  }
...
export async function buildPass(
  ...
  try {
    ...
    const storage = new UpstashAssetStorage(env, logger);
    ...
    if (!passRow) {
      logger.error('Pass not found', new Error('PASS_NOT_FOUND'), { serialNumber, passTypeIdentifier });
    ...
    if (!passTypeLink) {
      logger.error('No pass type mapping found', new Error('CONFIG_ERROR'), { passTypeIdentifier });
      throw new Error(`Server configuration error for pass type ${passTypeIdentifier}`);
    }
    ...
    try {
      ...
      logger.error('Failed to load certificate bundle', err, { certRef, passTypeIdentifier, serialNumber });
    ...
    if (typeof (passRow as any).passData === 'string') {
      try {
        ...
        logger.error('Invalid JSON in passData from DB', e, { passTypeIdentifier, serialNumber });
    ...
    try {
      ...
      if (validationError instanceof z.ZodError) {
        const prettyErrorSummary = validationError.issues.map((e: ZodIssue) => `${e.path.join('.') || 'passData'}: ${e.message}`).join('; ');
      ...
      logger.error('Unknown error during pass data validation', validationError, { passTypeIdentifier, serialNumber, ticketStyle: passRow.ticketStyle });
    ...
    let processedPassData = { ...validatedPassData };
    
    if (!processedPassData.eventTicket) {
      logger.info('No pre-structured eventTicket found, creating from loose fields');
      ...
      if (primaryFields.length === 0) {
        ...
        logger.warn('No loose fields available for eventTicket, using description as fallback', {
          passTypeIdentifier,
          serialNumber
        });
      }
      ...
      logger.info('Using pre-structured eventTicket from passData', { passTypeIdentifier, serialNumber });
    }
    ...
    if (validatedPassData.nfc) {
      if (!certBundle.isEnhanced) {
        logger.warn('Attempted to add NFC data with a non-enhanced certificate. NFC will be omitted.', { passTypeIdentifier, serialNumber });
        delete passJsonContent.nfc;
      } else {
        // Additional validation for encryptionPublicKey format
        if (!validatedPassData.nfc.encryptionPublicKey || validatedPassData.nfc.encryptionPublicKey.trim() === '') {
          logger.error('NFC data provided but encryptionPublicKey is empty or invalid. Apple rejects passes with empty encryptionPublicKey.', new Error('INVALID_NFC_ENCRYPTION_KEY'), { passTypeIdentifier, serialNumber });
          throw new Error('INVALID_NFC_ENCRYPTION_KEY: encryptionPublicKey cannot be empty');
        }
        logger.info('Valid NFC data included from passData with enhanced certificate.', { passTypeIdentifier, serialNumber });
      }
    }
    ...
    logger.info('Attempting to fetch pass-specific icon.png', { path: passSpecificIconKey });
    try {
      const iconBuffer = await fetchVerifiedPngAsset(storage, passSpecificIconKey, 29, 29, logger);
      modelFiles['icon.png'] = Buffer.from(iconBuffer);
      logger.info('Added pass-specific icon.png using fetchVerifiedPngAsset');
    } catch (error: any) {
      logger.warn(`Failed to fetch/verify pass-specific icon.png (${passSpecificIconKey}): ${error.message}. Trying fallback.`);
    ...
    if (!modelFiles['icon.png']) {
      ...
      logger.info('Attempting to fetch global fallback icon.png', { path: globalFallbackIconKey });
      try {
        const fallbackBuffer = await fetchVerifiedPngAsset(storage, globalFallbackIconKey, 29, 29, logger);
        modelFiles['icon.png'] = Buffer.from(fallbackBuffer);
        logger.info('Added global fallback icon.png using fetchVerifiedPngAsset');
      } catch (error: any) {
        logger.warn(`Failed to fetch/verify global fallback icon.png (${globalFallbackIconKey}): ${error.message}`);
        // If this also fails, the check at line 313 will handle it.
      }
    }
    
    if (!modelFiles['icon.png']) {
      logger.error('No valid icon.png found after trying all fallbacks', new Error('MISSING_ICON'), { serialNumber, passTypeIdentifier });
    ...
    for (const suffix of ['@2x', '@3x']) {
      ...
      let added = false;
      const expectedDimensions = suffix === '@2x' ? { width: 58, height: 58 } : { width: 87, height: 87 };
      
      logger.info(`Attempting to fetch pass-specific ${filename}`, { path: passSpecificKey });
      try {
        const imageBuffer = await fetchVerifiedPngAsset(storage, passSpecificKey, expectedDimensions.width, expectedDimensions.height, logger);
        modelFiles[filename] = Buffer.from(imageBuffer);
        logger.info(`Added pass-specific ${filename} using fetchVerifiedPngAsset`);
        added = true;
      } catch (error: any) {
        logger.warn(`Failed to fetch/verify pass-specific ${filename} (${passSpecificKey}): ${error.message}. Trying fallback.`);
      }
      
      if (!added) {
        logger.info(`Attempting to fetch global fallback ${filename}`, { path: globalFallbackKey });
        try {
          const fallbackBuffer = await fetchVerifiedPngAsset(storage, globalFallbackKey, expectedDimensions.width, expectedDimensions.height, logger);
          modelFiles[filename] = Buffer.from(fallbackBuffer);
          logger.info(`Added global fallback ${filename} using fetchVerifiedPngAsset`);
          // 'added' remains false if this path is taken, but the file is added.
          // The 'added' flag was primarily for the outer if condition.
        } catch (error: any) {
          logger.warn(`Failed to fetch/verify global fallback ${filename} (${globalFallbackKey}): ${error.message}`);
        }
      }
    }
    // --- End Icon Handling ---
    ...
    try {
      const logoBuffer = await fetchVerifiedPngAsset(storage, logoKey, 160, 50, logger);
      modelFiles['logo.png'] = Buffer.from(logoBuffer);
      logger.info('Added logo.png using fetchVerifiedPngAsset');
    } catch (error: any) {
      logger.error(`Failed to fetch/verify mandatory logo.png (${logoKey}): ${error.message}`, new Error('MANDATORY_LOGO_FAILURE'), { serialNumber, passTypeIdentifier });
      throw new Error(`Mandatory logo.png (${logoKey}) could not be processed: ${error.message}`);
    }
    
    logger.info('Attempting to fetch optional logo@2x.png', { path: logo2xKey });
    try {
      const logo2xBuffer = await fetchVerifiedPngAsset(storage, logo2xKey, 320, 100, logger);
      modelFiles['logo@2x.png'] = Buffer.from(logo2xBuffer);
      logger.info('Added logo@2x.png using fetchVerifiedPngAsset');
    } catch (error: any) {
      logger.warn(`Optional logo@2x.png (${logo2xKey}) could not be processed or was not found: ${error.message}`);
    ...
    if (passRow.poster && certBundle.isEnhanced) {
      logger.info('Processing poster-specific assets (background image)', { poster: true, isEnhanced: true, serialNumber, passTypeIdentifier });
      const background2xKey = `${passRow.passTypeIdentifier}/${passRow.serialNumber}/background@2x.png`;
      logger.info('Attempting to fetch background@2x.png for poster', { path: background2xKey });
      try {
        const bg2xBuffer = await fetchVerifiedPngAsset(storage, background2xKey, 180 * 2, 220 * 2, logger);
        modelFiles['background@2x.png'] = Buffer.from(bg2xBuffer);
        logger.info('Added background@2x.png for poster pass using fetchVerifiedPngAsset');
      } catch (error: any) {
        logger.warn(`Optional background@2x.png for poster (${background2xKey}) could not be processed or was not found: ${error.message}`);
    ...
    if (!LazyPKPass) {
      logger.info('Fallback: dynamically loading passkit-generator (preload may have failed)...');
      try {
        LazyPKPass = (await import('passkit-generator')).PKPass;
        logger.info('passkit-generator loaded via fallback.');
      } catch (error: any) {
        logger.error('Failed to load passkit-generator', error, { passTypeIdentifier, serialNumber });
    ...
    if (pkpassData instanceof ArrayBuffer) {
      ...
      return pkpassData;
    } else if (pkpassData instanceof Uint8Array) {
    ...
    logger.error('Unexpected pass buffer type', new Error('UNEXPECTED_BUFFER_TYPE'), { serialNumber, passTypeIdentifier });
    throw new Error('Unexpected pass buffer type generated by passkit-generator');
  } catch (error) {
    const log = logger || console;
    log.error('Failed to build pass', error instanceof Error ? error : new Error(String(error)), { passTypeIdentifier, serialNumber });
    throw error;
  }
}
