...
import { storeCertBundle, invalidateCertCache } from '../passkit/certs';
...
if (!user || !pass) {
  logger.error("Admin Basic Auth secrets not set. HONOKEN_ADMIN_USERNAME or HONOKEN_ADMIN_PASSWORD missing.", {});
...
let attemptedUsername: string | undefined = undefined;
...
adminApp.put('/admin/certs/:certRef', async (c) => {
  ...
  if (!certRef) {
    throw new HTTPException(400, { message: 'certRef path parameter is required.' });
  }
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      logger.error('Certificate rotation validation failed', error, { certRef });
      return c.json({
        success: false,
...
adminApp.post('/admin/invalidate/certs/:certRef', async (c) => {
  ...
  try {
    ...
    const exists = await db.query.certs.findFirst({
    ...
    if (!exists) {
      ...
      throw new HTTPException(404, { message: `Certificate with certRef '${certRef}' not found.` });
    }
