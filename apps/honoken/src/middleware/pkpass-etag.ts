...
import { ensureStoredEtag, type PassWithContent } from '../db/etag'; // Import the new types
...
async function queryPassWithContent(
  ...
  // Current implementation: Query just the passes table
  const passRow = await db.query.passes.findFirst({
    where: (passes: any, { eq, and }: any) => and(
  ...
  const passWithContent: PassWithContent = {
    ...
    passContentId: passRow.passContentId, // Will be populated when FK exists
    updatedAt: passRow.updatedAt,
    // TODO: Add joined passContent when the table exists
    // passContent: {
    //   id: result.contentId,
    //   description: result.description,
    //   organizationName: result.organizationName,
    //   updatedAt: result.contentUpdatedAt,
    //   // ... other fields from the JOIN
    // }
  };
 ...
 */
export const pkpassEtagMiddleware: MiddlewareHandler<PkpassContext> = async (c, next) => {
  ...
  const passTypeIdentifier = c.req.param('passTypeIdentifier') as string | undefined;
  ...
  try {
    ...
    const passWithContent = await queryPassWithContent(db, passTypeIdentifier, serialNumber, logger);
    ...
    // Compute ETag using both pass metadata and content
    const etagValue = await ensureStoredEtag(db, passWithContent, logger);
    ...
    if (ifModifiedSinceHeader) {
      try {
        ...
        logger.warn('Invalid If-Modified-Since header received', { ifModifiedSinceHeader, error: e });
      }
    }
    ...
    logger.error(`PKPass ETag middleware error`, error instanceof Error ? error : new Error(String(error)));
    throw error;
  }
}; 