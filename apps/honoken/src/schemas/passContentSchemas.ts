...
export const PassFieldSchema = z.object({
  ...
  value: z.union([
    ...
    z.iso.datetime({
      error: "Field 'value' (date) must be a valid ISO 8601 date string.",
    }),
  ]),
...
export const BarcodeSchema = z
  .object({
    ...
    messageEncoding: z
      .string()
      .min(1, { error: "Barcode 'messageEncoding' cannot be empty." }) // e.g., "iso-8859-1"
...
export const PassDataEventTicketSchema = z
  .object({
    // REQUIRED: Apple's absolute minimum requirements
    description: z
      .string()
      .min(1, { error: "Pass 'description' is required by Apple." }),
    organizationName: z
      .string()
      .min(1, { error: "Pass 'organizationName' is required by Apple." }),
    ...
    eventDateISO: z
      .string()
      .datetime({ error: 'eventDateISO must be a valid ISO 8601 date string.' })
      .optional(),
    ...
    nfc: z
      .object({
        ...
        encryptionPublicKey: z
          ...
          .min(1, {
            error:
              'NFC encryptionPublicKey cannot be empty - either provide a valid Base64-encoded ECDH P-256 public key or omit the nfc object entirely',
          })
          .regex(/^[A-Za-z0-9+/]+={0,2}$/, {
            error:
              'NFC encryptionPublicKey must be a valid Base64-encoded string',
          })
          .refine(
            (key) => {
            ...
            {
              message:
                'NFC encryptionPublicKey appears to be invalid length for ECDH P-256 public key',
            }
          ),
      })
      .optional(),
    ...
    relevantDate: z.iso
      .datetime({
        error: 'Relevant date must be a valid ISO 8601 date string.',
      })
      .optional(),
    ...
    semanticTags: z
      .record(
        z.string(),
        z.union([z.string(), z.number(), z.boolean(), z.string().datetime()])
      )
      .optional(),
  })
