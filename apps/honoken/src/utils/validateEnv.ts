...
export function validateEnv(env: Partial<Env>): asserts env is Env {
  const requiredVars = [
    ...
    // Redis
    { names: ['UPSTASH_REDIS_REST_URL'], error: 'UPSTASH_REDIS_REST_URL is required for asset storage' },
    { names: ['UPSTASH_REDIS_REST_TOKEN'], error: 'UPSTASH_REDIS_REST_TOKEN is required for asset storage' },
    // Admin auth
    { names: ['HONOKEN_ADMIN_USERNAME'], error: 'HONOKEN_ADMIN_USERNAME is required for admin endpoints' },
    { names: ['HONOKEN_ADMIN_PASSWORD'], error: 'HONOKEN_ADMIN_PASSWORD is required for admin endpoints' },
    // Encryption - versioned keys
    { names: ['HONOKEN_ENCRYPTION_KEY_V1'], error: 'HONOKEN_ENCRYPTION_KEY_V1 is required for certificate encryption' },
    { names: ['HONOKEN_ENCRYPTION_KEY_CURRENT'], error: 'HONOKEN_ENCRYPTION_KEY_CURRENT is required to specify the active encryption key version' },
    // Service metadata
    { names: ['SERVICE_NAME'], error: 'SERVICE_NAME is required for logging and monitoring' },
    { names: ['ENVIRONMENT'], error: 'ENVIRONMENT is required for logging and monitoring' },
    // PostHog
    { names: ['POSTHOG_PROJECT_API_KEY'], error: 'POSTHOG_PROJECT_API_KEY is required for analytics and error tracking' },
  ];
  ...
  if (currentVersion) {
    ...
    const versionedKey = (env as any)[versionedKeyName];
  ...
  const numericVars = [
    ...
    { name: 'POSTHOG_FLUSH_INTERVAL', min: 1000, max: 300000 },
  ];
  ...
  if (env.LOG_SAMPLE_SUCCESS_RATE) {
    const rate = parseFloat(env.LOG_SAMPLE_SUCCESS_RATE);
