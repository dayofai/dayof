...
describe('Certificate and Cache Management', () => {
    ...
    Object.defineProperty(global, 'crypto', {
      value: {
        subtle: {
          ...
          decrypt: vi
            .fn()
            .mockResolvedValue(new TextEncoder().encode('{"test": "data"}')),
  ...
  describe('Certificate Loading with Cache', () => {
    ...
    it('should invalidate stale cache entries', async () => {
      const { loadCertBundle, certCache } = await import(
        '../../src/passkit/certs'
      );
    ...
    it('should handle missing certificate gracefully', async () => {
      ...
      await expect(
        loadCertBundle('nonexistent-cert', mockEnv, mockLogger)
    ...
    it('should handle decryption failures', async () => {
      ...
      (global.crypto.subtle.decrypt as any).mockRejectedValueOnce(
  ...
  describe('Cache Invalidation', () => {
    it('should manually invalidate cache entries', async () => {
      const { invalidateCertCache, certCache } = await import(
        '../../src/passkit/certs'
      );
    ...
    it('should handle invalidation of non-existent cache entries gracefully', async () => {
      ...
      expect(() =>
        invalidateCertCache('nonexistent-cert', mockLogger)
      ).not.toThrow();
    });
  });
  ...
  describe('Certificate Storage', () => {
    it('should encrypt and store certificate bundle', async () => {
      ...
      expect(insertCall.encryptedBundle).toMatch(
        /^v\d+:[A-Za-z0-9+/=]+:[A-Za-z0-9+/=]+$/
      );
    });
  });
