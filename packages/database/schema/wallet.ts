...
export const walletApnsKey = pgTable(
  ...
  (t) => [
    index('idx_wallet_apns_key_team_active').on(t.teamId, t.isActive),
    uniqueIndex('uq_wallet_apns_key_team_key').on(t.teamId, t.keyId),
    uniqueIndex('uq_wallet_apns_key_key_ref').on(t.keyRef),
  ]
);
...
export const walletPassType = pgTable(
  ...
  (t) => [
    index('idx_wallet_pass_type_cert_ref').on(t.certRef),
    uniqueIndex('uq_wallet_pass_type_identifier').on(t.passTypeIdentifier),
  ]
);
...
export const walletRegistration = pgTable(
  ...
  (t) => [
    index('idx_wallet_registration_pass_ref').on(
      t.passTypeIdentifier,
      t.serialNumber
    ),
    index('idx_wallet_registration_device_active').on(
      t.deviceLibraryIdentifier,
      t.active
    ),
    uniqueIndex('uq_wallet_registration_device_pass').on(
      t.deviceLibraryIdentifier,
      t.passId
    ),
    index('wallet_registration_pass_id_idx').on(t.passId),
  ]
);
...
export const walletPassContent = pgTable(
  ...
  (t) => [uniqueIndex('uq_wallet_pass_content_pass_id').on(t.passId)]
);
